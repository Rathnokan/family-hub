...
| **Phase** | **v0.8.0 "Home Maintenance" — Phase A IN PROGRESS (A1–A3 done + live-tested 2026-07-21; A4 next).** Plan: **[docs/PLAN-v0.8.0.md](docs/PLAN-v0.8.0.md)**. **A1** foundations plan · **A2** module framework + event bus (all modules gateable via OptionsFlow+reload; `event_bus.FamilyHubBus`; meals gated end-to-end — chores/rewards toggles experimental until A6) · **A3** versioned export/import (`DATA_SCHEMA_VERSION 3`, `export_data`/`import_data`, validate-then-swap, 12/12 live test pass). **Next: A4 maintenance backend** — new `maintenance` store domain, `_maintenance_schedule.py` derived-state (no task_instances), ~19 `maintenance_*` services, seed-loader interface (library ships empty). **A4 decision (2026-07-21): `assign` = publish `external_task_offer`/`_revoke` to the bus only (designed no-op until D5 chores bridge); NO standalone in-module assignee field.** Then A5 migration + A6 chores/rewards gating audit. Scope docs live in the user's Claude project knowledge. ⚠️ v0.7.6 phone surfaces still NOT device-tested. |
