...
| **Phase** | **v0.8.0 "Home Maintenance" — Phase A COMPLETE (A1–A6 done + live-tested, on `main`, CI green). Not yet released/tagged.** Plan: **[docs/PLAN-v0.8.0.md](docs/PLAN-v0.8.0.md)**. **A1** foundations plan · **A2** module framework + event bus (`event_bus.FamilyHubBus`, OptionsFlow+reload) · **A3** versioned export/import (`DATA_SCHEMA_VERSION 3`, validate-then-swap) · **A4** maintenance backend (own store domain, `_maintenance_schedule.py` derived-state, no task_instances, ~19 services; `assign` = bus offer/revoke only) · **A5** chores→maintenance migration + `_chore_is_maintenance` seam removal (live-migrated the one Maintenance chore) · **A6** chores/rewards gating (both toggles real; register-then-remove services, gated sensors/tick/model/tabs; core points survive both-off). **Phase B COMPLETE (2026-07-25):** seed library shipped to `custom_components/family_hub/seed_library.json` (v3.0.0, 97 tasks + 15 big-ticket assets, Tucson-tuned); provenance in `docs/research-phase-b/`. ⚠️ Library is dict-shaped → A4 stub loader no-ops until D1 (safe). D1 integration deltas catalogued in **[docs/PLAN-v0.8.0.md](docs/PLAN-v0.8.0.md) §9**. **Phase C COMPLETE (2026-08-23).** **C1** admin IA wireframes (five-rail nav, module-off greys in place, Home Care admin sub-tabs, points economy in Rewards) — ⚠️ 6 deltas from shipped A4–A6 in **[C1-DECISIONS.md](docs/design-phase-c/C1-DECISIONS.md)**, read before D3. **C2** Home Care room handoff — spec + 28 screens + **[C2-DECISIONS.md](docs/design-phase-c/C2-DECISIONS.md)** (⚠️ 6 items of new D-phase work incl. a hub-wide parent-PIN gate keyed to `hass.user.is_admin` with server-side verification), read before D2. **D1 COMPLETE (2026-08-25) — deployed to Samba, pending live-test.** Seed library is LIVE: dict loader, Home Profile schema (19 presence + 7 variant selects, blank = everything off ⇒ 56 universal tasks), all 22 applicability tags → `_TAG_PREDICATES` (fails closed on unknown), `climate_tags` second gate, **multi-occurrence `seasonal_anchor`** (now a LIST — required by gutter_clean's June & October override), 24/24 anchor strings parse, `desert_southwest` overrides applied (11 tasks), assets profile-gated, products seeded as untracked stubs, `all_tasks` + assets/products/completions/profile on the `maintenance_due` model payload (~128 KB). **3 latent A4 bugs fixed** (see BUGS.md). Verified by 185 offline checks (`d1_harness.py` 117 + `d1_lifecycle.py` 68, in the session scratchpad). **Next: D2 (room UI from the C2 spec).** ⚠️ Deferred cosmetic: per-theme personal-page section-hiding when chores/rewards off (see BUGS.md). ⚠️ v0.7.6 phone surfaces still NOT device-tested. |
...
⚠️ **`seed_library.json` is shipped integration code and must be copied to Samba like any `.py` file.** It was missed after Phase B — the share held the 3-byte A4 stub (`[]`) until D1, which would have made every seed test silently no-op. Check its size on the share (~168 KB) if seeding does nothing. (`family_hub_data.json` is the only file that stays off-limits — that's user data.)
