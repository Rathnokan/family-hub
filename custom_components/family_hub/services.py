...
async def async_setup_services(hass: HomeAssistant, coordinator: FamilyHubCoordinator) -> None:
    ...
    # ------------------------------------------------------------------
    # Meals room (v0.8.0). The card resolves the meal library client-side and
    # passes already-resolved fields (sides, variant, protein). Every handler
    # mutates the store then refreshes the coordinator so the new meals model
    # propagates to the card.
    # ------------------------------------------------------------------
