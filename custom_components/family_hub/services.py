...
async def async_setup_services(
    ...
    v0.8.0: `enabled` is the set of enabled module ids. Core services are always
    registered; each gateable module's services (via its registrar in modules.py)
    register only when the module is enabled. None = all modules on (safe default).
    """

    store = coordinator.store
    if enabled is None:
        from .modules import MODULE_IDS
        enabled = frozenset(MODULE_IDS)
