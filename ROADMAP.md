...
Next: **Phase C** design (admin IA rethink + maintenance room prototype → handoff spec — now unblocked, real seed data exists), non-Code. Then **Phase D** (Code): D1 seed import + Home-Profile applicability + sensors, D2 room UI from the design spec, D3 admin tab, D4 notifications, D5 chores bridge (the `external_task_*` bus subscriber). D1 can start now (needs the library, not the design). See [docs/PLAN-v0.8.0.md](docs/PLAN-v0.8.0.md) §9 + the implementation-plan doc in project knowledge.
